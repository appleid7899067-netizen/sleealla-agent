...
func initializeLLM(apiKey, baseURL, modelName string) (llms.Model, error) {
	return openai.New(
...
func initializeMCPTools(client *mcp.Client, mcpGatewayURL string) ([]tools.Tool, error) {
	...
	for i, tool := range mcpTools.Tools {
		...
		toolBelt[i] = &DuckDuckGoTool{clientSession: cs, mcpTool: tool, args: args}
	}
